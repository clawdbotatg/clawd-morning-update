...
// Collapsible card list: headlines/blurbs stay scannable, the supporting
// tweets expand on tap. Summary line = count + first few handles as a teaser.
export function cardsBlock(tweets, renderCard = tweetCard) {
  const who = [...new Set(tweets.map((t) => "@" + t.author))];
  const teaser = who.slice(0, 3).join(" ") + (who.length > 3 ? " …" : "");
  return `<details class="tweets"><summary><span class="sumcount">${tweets.length} tweet${tweets.length === 1 ? "" : "s"}</span> <span class="sumwho">${esc(teaser)}</span></summary>
<div class="cards">${tweets.map((t) => renderCard(t)).join("\n")}</div></details>`;
}
...
export const CSS = `
  ...
  * { box-sizing:border-box; margin:0; }
  ...
  details.tweets { margin-top:6px; }
  details.tweets summary { cursor:pointer; list-style:none; -webkit-user-select:none; user-select:none;
                           color:var(--muted); font-size:.85rem; padding:6px 0; }
  details.tweets summary::-webkit-details-marker { display:none; }
  details.tweets summary::before { content:"▸ "; color:var(--accent); }
  details.tweets[open] summary::before { content:"▾ "; }
  details.tweets .sumcount { color:var(--accent); font-weight:600; }
